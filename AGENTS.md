...
- **必须修补 initrd**：Ubuntu Server ISO 的 initrd 包含 `/scripts/init-bottom/live-server` 脚本，该脚本硬编码挂载 `ubuntu-server-minimal.squashfs` 和 `ubuntu-server-minimal.ubuntu-server.squashfs`。自定义 LiveOS ISO 没有这些文件，导致 mount 失败后 /dev /proc /sys /run 无法挂载，最终 kernel panic。使用 `scripts/patch_initrd.sh` 修补 initrd 移除这些 squashfs 挂载操作
...
- initrd 包含 live-server 脚本导致 kernel panic：Ubuntu Server ISO 的 initrd 里 `/scripts/init-bottom/live-server` 硬编码挂载 `ubuntu-server-minimal.squashfs`，自定义 LiveOS ISO 没有这些文件，mount 失败后 /dev /proc /sys /run 无法挂载。必须用 `scripts/patch_initrd.sh` 修补 initrd
