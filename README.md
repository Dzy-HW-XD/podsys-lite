...
# 2. 在黄金机上执行
bash /root/prepare_liveos.sh /tmp/podsys-liveos
```
...
产出三个文件：
```
/tmp/podsys-liveos/vmlinuz              # ARM64 内核
/tmp/podsys-liveos/initrd               # 包含网卡驱动的 initramfs
/tmp/podsys-liveos/filesystem.squashfs  # 根文件系统压缩镜像
```
...
```bash
# 拷贝到管理节点
scp /tmp/podsys-liveos/{vmlinuz,initrd,filesystem.squashfs} \
